...
<template>
	<div class="min-h-screen bg-slate-900 text-white flex flex-col items-center">
				...
				<div class="flex flex-col items-center order-first md:order-none w-full md:w-auto">
					...
					<div v-if="G" class="flex items-center gap-2 md:gap-3 text-[10px] md:text-xs text-slate-400 mb-0.5 md:mb-1 min-h-5 md:min-h-6">
						...
						<template v-else-if="currentPlayerSuit">
							...
							>
								<span class="capitalize">{{ currentPlayerSuit }}</span
								>'s turn
							</span>
						</template>
					</div>
		...
		<div class="w-[1200px] flex flex-col items-center p-2 md:p-6">
		...
		<!-- Fixed hand tray: content teleported from GameBoard (always in DOM so Teleport works) -->
		<div id="game-hand-tray" class="fixed bottom-0 left-0 right-0 z-10 min-h-[180px] pointer-events-none" />
